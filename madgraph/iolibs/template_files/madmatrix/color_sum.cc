...
%(color_matrix_lines)s
  ...
  // The fptype_colour version is the default used by kernels (supporting mixed floating point mode also in blas)
  static __device__ fptype_colour s_pNormalizedColorMatrix2[ncolor * ncolor];
#endif
  ...
  void createNormalizedColorMatrix()
  {
    ...
    {
      first = false;
      constexpr NormalizedColorMatrix<fptype_colour> normalizedColorMatrix2;
      gpuMemcpyToSymbol( s_pNormalizedColorMatrix2, normalizedColorMatrix2.value, ncolor * ncolor * sizeof( fptype_colour ) );
    }
  }
#endif
...
#else
    const cxtype_amp_sv* jamp_sv = allJamp_sv;
#endif
    // Loop over icol
    for( int icol = 0; icol < ncolor; icol++ )
    {
...
#else
      fptype_colour_sv jampRi_sv = (fptype_colour_sv)( cxreal( jamp_sv[icol] ) );
      fptype_colour_sv jampIi_sv = (fptype_colour_sv)( cximag( jamp_sv[icol] ) );
#endif
...
#else
        fptype_colour_sv jampRj_sv = (fptype_colour_sv)( cxreal( jamp_sv[jcol] ) );
        fptype_colour_sv jampIj_sv = (fptype_colour_sv)( cximag( jamp_sv[jcol] ) );
#endif
  ...
  __global__ void
  convertD2F_Jamps( fptype_colour* allJampsFpt2,  // output: jamp[2][ncolor][ihel][nevt] for one specific helicity ihel
                    const fptype_amp* allJamps, // input: jamp[2][ncolor][ihel][nevt] for one specific helicity ihel
  ...
  convertF2D_MEs( fptype* allMEs,             // output: allMEs[nevt] for one specific helicity
                  const fptype_colour* allMEsFpt2 ) // input: allMEs[nevt] for one specific helicity
  {
    const int ievt = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  color_sum_blas( fptype* ghelAllMEs,           // output: allMEs super-buffer[nhel][nevt], add |M|^2 separately for each helicity
                  const fptype_amp* ghelAllJamps,   // input: allJamps super-buffer[2][ncol][nhel][nevt] for nhel good helicities
                  fptype_colour* ghelAllBlasTmp,      // tmp: allBlasTmp super-buffer for nhel good helicities
                  ...
                  const int gputhreads )        // input: cuda gputhreads
  {
    ...
    static fptype_colour* devNormColMat = nullptr;
    if( !devNormColMat ) gpuGetSymbolAddress( (void**)&devNormColMat, s_pNormalizedColorMatrix2 );

#if defined MGONGPU_FPTYPE_DOUBLE and defined MGONGPU_FPTYPE2_FLOAT
    // Mixed precision mode: need two fptype_colour[2*ncolor*nhel*nevt] buffers and one fptype_colour[nhel*nevt] buffers for the nhel helicities
    fptype_colour* ghelAllZtempBoth = ghelAllBlasTmp;                                         // start of first fptype_colour[ncolor*2*nhel*nevt] buffer
    fptype_colour* ghelAllJampsFpt2 = ghelAllBlasTmp + ncolor * mgOnGpu::nx2 * nhel * nevt;   // start of second fptype_colour[ncolor*2*nhel*nevt] buffer
    fptype_colour* ghelAllMEsFpt2 = ghelAllBlasTmp + 2 * ncolor * mgOnGpu::nx2 * nhel * nevt; // start of fptype_colour[nhel*nevt] buffer
    // Convert jamps from double to float
    for( int ighel = 0; ighel < nhel; ighel++ )
    {
      const fptype_amp* hAllJamps = ghelAllJamps + ighel * nevt;    // jamps for a single helicity ihel
      fptype_colour* hAllJampsFpt2 = ghelAllJampsFpt2 + ighel * nevt; // jamps for a single helicity ihel
      gpuLaunchKernelStream( convertD2F_Jamps, gpublocks, gputhreads, ghelStreams[ighel], hAllJampsFpt2, hAllJamps, nhel );
    }
    // Real and imaginary components
    const fptype_colour* ghelAllJampsReal = ghelAllJampsFpt2;
    const fptype_colour* ghelAllJampsImag = ghelAllJampsFpt2 + ncolor * nhel * nevt;
#else
    // Standard single or double precision mode: need one fptype_colour[ncolor*2*nhel*nevt] buffer
    static_assert( std::is_same<fptype_colour, fptype>::value );
    fptype_colour* ghelAllZtempBoth = ghelAllBlasTmp; // start of fptype_colour[ncolor*2*nhel*nevt] buffer
    fptype_colour* ghelAllMEsFpt2 = ghelAllMEs;
    // Real and imaginary components
    const fptype_colour* ghelAllJampsReal = ghelAllJamps;                        // this is not a cast (the two types are identical)
    const fptype_colour* ghelAllJampsImag = ghelAllJamps + ncolor * nhel * nevt; // this is not a cast (the two types are identical)
#endif
    // Real and imaginary components
    fptype_colour* ghelAllZtempReal = ghelAllZtempBoth;
    fptype_colour* ghelAllZtempImag = ghelAllZtempBoth + ncolor * nhel * nevt;
    ...
    fptype_colour alpha1 = 1;
    fptype_colour beta1 = 0;
    ...
    fptype_colour alpha2 = 1;
    fptype_colour beta2 = 1;
    ...
    for( int ighel = 0; ighel < nhel; ighel++ )
    {
      ...
      fptype_colour* hAllMEsFpt2 = ghelAllMEsFpt2 + ighel * nevt; // MEs for a single helicity ihel      
...
#if defined MGONGPU_FPTYPE_DOUBLE and defined MGONGPU_FPTYPE2_FLOAT
      gpuMemset( ghelAllBlasTmp, 0, nGoodHel * nevt * ( 2 * ncolor * mgOnGpu::nx2 + 1 ) * sizeof( fptype_colour ) );
#else
      gpuMemset( ghelAllBlasTmp, 0, nGoodHel * nevt * ( ncolor * mgOnGpu::nx2 ) * sizeof( fptype_colour ) );
#endif
