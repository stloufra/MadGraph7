    ...
    def generate_helas_call(self, argument, gauge_check=False):
        ...
        if isinstance(argument, helas_objects.HelasWavefunction) and \
            ...
            if argument.get('spin') == 1:
                ...
                # Fermions: add flavor argument only for merged-particle models
                if argument.get('flavor'):
                    # Merged particle: pass the flavor index
                    call_ferm = call[:-1] + ',%d)'
                    call_function = lambda wf, c=call_ferm: c % \
                                    (wf.get('me_id')-1,
                                     ...
                                     # For fermions, need particle/antiparticle
                                     -(-1)**wf.get_with_flow('is_part'),
                                     # Flavor index for the merged particle
                                     wf.get('flavor')[0])
                else:
                    # Standard (non-merged) fermion: no flavor argument
                    call_function = lambda wf: call % \
                                    (wf.get('me_id')-1,
                                     wf.get('number_external')-1,
                                     wf.get('mass'),
